...
for attr_list in attr_lists:
    ...
    if isError(resp[0]):
        # Hacky way to do this.. if we get a 404, that means we dont this Endpoint in ISE.
        ...
        if err_msg == '405':
            ...
            demisto.info("PANW_IOT_3RD_PARTY_BASE Creating new Endpoint %s" % mac)
        ...
        try:
            ...
            demisto.info("PANW_IOT_3RD_PARTY_BASE Updating existing Endpoint %s" % mac)
            ...
            if isError(resp[0]):
                return_results("Failed to update Custom Attributes for Endpoint %s" % mac)  # log to the war room
